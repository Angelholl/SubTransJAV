#!/usr/bin/env python3
"""
Console utilities for SubTransJAV.
...
Usage:
    from subtransjav.utils.console import ensure_utf8_console, safe_print
...
def ensure_utf8_console() -> None:
    ...
    Example:
        ...
        from subtransjav.utils.console import ensure_utf8_console
        ensure_utf8_console()
    """
...
def safe_print(*args: Any, **kwargs: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            print(ascii_output, sep=sep, end=end, file=file, flush=flush)
...
def suppress_dependency_warnings() -> None:
    ...
    Example:
        from subtransjav.utils.console import suppress_dependency_warnings
...
def ensure_numba_cache_dir() -> None:
    ...
    cache_dir = os.path.join(base, "subtransjav", "numba_cache")
    try:
...
def setup_console() -> None:
    """
    Complete console setup for SubTransJAV.

    Combines UTF-8 encoding fix, warning suppression, and environment
    setup.  Call this at the start of any entry point.

    Example:
        from subtransjav.utils.console import setup_console
        setup_console()

        # Now safe to import other modules
        from subtransjav.main import main
    """
    ensure_utf8_console()
    suppress_dependency_warnings()
...
def check_extras_installed(extra_name: str, required_packages: list[str]) -> tuple[bool, list[str]]:
    ...
    Example:
        ...
        if not installed:
            ...
            print(f"Install with: pip install subtransjav[gui]")
    """
    missing = []
...
def print_missing_extra_error(
    ...
    print(f"  pip install subtransjav[{extra_name}]")
    print()
    print("Or install all features:")
    print("  pip install subtransjav[all]")
    print("=" * 60)
    print()
