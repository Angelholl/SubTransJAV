...
DEFAULT_SETTINGS = {
    'version': 'v1.0.0',
    '_comment': 'SubTransJAV user settings',
    'provider': 'deepseek',
    'model': None,
...
def get_settings_path() -> Path:
    ...
    settings_path = base / 'SubTransJAV' / 'translate' / 'settings.json'
    ...
    if settings:
        for key, value in settings.items():
            if key not in ('version', '_comment'):
                if key == 'token_budget' and isinstance(value, dict) and isinstance(config.get('token_budget'), dict):
                    # Deep-merge: allow users to override individual token budget values
                    config['token_budget'] = {**config['token_budget'], **value}
                else:
                    config[key] = value
